...
for (i.year in 1:2) {
  ...
  eth_age = 10*eth + age,
  inc_age = 10*inc + age,
  reg_eth_inc = 100*reg + 10*eth + inc,
  stt_eth_inc = 100*stt + 10*eth + inc,
  reg_eth_age = 100*reg + 10*eth + age,
  reg_inc_age = 100*reg + 10*inc + age,
  stt_eth_age = 100*stt + 10*eth + age,
  stt_inc_age = 100*stt + 10*inc + age,
  eth_inc_age = 100*eth + 10*inc + age,
  reg_eth_inc_age = 1000*reg + 100*eth + 10*inc + age,
  stt_eth_inc_age = 1000*stt + 100*eth + 10*inc + age,
  z_inc = rescale(inc),
  ...
  tmp$ones <- 1
  tmp <- merge(x = select_(D, grp, ix), y = tmp, by="grp", all.x=T)  
  
  M.cps[[i.year]] <- stan_vote_regression(D, tmp, cat(c("CPS_Turnout_Model",years[i.year]),sep="_"))
  ...
  tmp$ones <- 1
  tmp <- merge(x = select_(D, grp, ix), y = tmp, by="grp", all.x=T)  
  rename(tmp,vote = rvote)

  M.cps[[i.year]] <- stan_vote_regression(D, tmp, cat(c("Pew_Vote_Choice_Model",years[i.year]),sep="_"))
...
D <- merge(x=D, y=data_pop_info, by="grp", all.x=T)
D <- D[order(D$ix),]
...
for (i in 1:n.stt) {
  ...
  ok <- D$stt==i
  D$turn2004[ok] <- weighted_correction(a=D$turn2004.M[ok], w=D$pop2004[ok], x=dat.stt[as.character(i), "vote2004"])$corrected
  D$turn2008[ok] <- weighted_correction(a=D$turn2008.M[ok], w=D$pop2008[ok], x=dat.stt[as.character(i), "vote2008"])$corrected
  D$vote2004[ok] <- weighted_correction(a=D$vote2004.M[ok], w=D$pop2004[ok]/sum(D$pop2004[ok]), x=dat.stt[as.character(i), "rep2004"])$corrected
  D$vote2008[ok] <- weighted_correction(a=D$vote2008.M[ok], w=D$pop2008[ok]/sum(D$pop2008[ok]), x=dat.stt[as.character(i), "rep2008"])$corrected
}
...
remove(list=objects())
setwd("~/Documents/Gelman Research/Replication/Original Research Round/")
