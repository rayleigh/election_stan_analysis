...
D2 <- summarize(group_by(D, plot.grp), vote2008 = sum(vote2008 * pop2008)/sum(pop2008), stt = unique(stt), inc = unique(inc))
D3 <- summarize(group_by(D[D$eth==1,], plot.grp), vote2008 = sum(vote2008 * pop2008)/sum(pop2008), stt = unique(stt), inc = unique(inc))
