...
parameters{
    ...
    vector[N_age] v_age_z_inc;
    vector[N_age] v_age_Intercept;
    vector<lower=0>[2] sigma_age;
    cholesky_factor_corr[2] L_Rho_age;
}
transformed parameters{
    vector[2] vec_v_age_Intercept_v_age_z_inc[N_age];
    ...
    for (j in 1:N_age) {
        vec_v_age_Intercept_v_age_z_inc[j,1] <- v_age_Intercept[j];
        vec_v_age_Intercept_v_age_z_inc[j,2] <- v_age_z_inc[j];
    }
...
model{
    ...
    for (i in 1:N) {
        p[i] <- Intercept + b_z_inc * z_inc[i] + b_z_incstt * z_incstt[i] + b_z_trnprv * 
            ...
            v_age_Intercept[age[i]] + v_age_z_inc[age[i]] * z_inc[i];
    }
