...
generate_unique_index_list <- function(num_vals, upper_limit)
{
  index_list <- c()
  while(length(index_list) < num_vals)
  {
    index_list <- unique(c(index_list, ceiling(runif(num_vals, 0, upper_limit))))
  }
  return(index_list[1:num_vals])
}
